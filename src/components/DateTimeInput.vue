...
const displayValue = ref(formatDateTime(props.modelValue))
...
function formatDateTime(value) {
  ...
  const local = raw.match(/^(\d{4})-(\d{2})-(\d{2})[T\s](\d{2}):(\d{2})/)
...
function parseDateTime(value) {
  ...
  const th = raw.match(/^(\d{1,2})[/-](\d{1,2})[/-](\d{4})\s+(\d{1,2}):(\d{2})$/)
  if (th) return toLocalValue(Number(th[3]), Number(th[2]), Number(th[1]), Number(th[4]), Number(th[5]))

  const local = raw.match(/^(\d{4})-(\d{1,2})-(\d{1,2})[T\s](\d{1,2}):(\d{2})$/)
  if (local) return toLocalValue(Number(local[1]), Number(local[2]), Number(local[3]), Number(local[4]), Number(local[5]))

  return null
}

function toLocalValue(year, month, day, hour, minute) {
...
function commit({ notifyChange = false } = {}) {
  const next = parseDateTime(displayValue.value)
  if (next === null) {
    displayValue.value = formatDateTime(props.modelValue)
    return
  }
  emit('update:modelValue', next)
  displayValue.value = formatDateTime(next)
